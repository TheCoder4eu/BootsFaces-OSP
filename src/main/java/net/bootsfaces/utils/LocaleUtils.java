	...
	private static String translateFormat(String formatString, Map<String, String> mapping) {
		...
		for (; i < formatString.length(); i++) {
			...
			if (i > 0 && lastChar != currentChar) {
				resultString += mapSubformat(formatString, mapping, beginIndex, i);
				beginIndex = i;
			}
			lastChar = currentChar;
		}

		return resultString + mapSubformat(formatString, mapping, beginIndex, i);
	}
