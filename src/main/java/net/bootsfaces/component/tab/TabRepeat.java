...
public class TabRepeat extends UINamingContainer {
	...
	private boolean isNestedInIterator() {
		...
		while (parent != null) {
			if (parent instanceof UIData || parent instanceof TabRepeat) {
				return true;
			}
	...
	public int process(FacesContext faces, PhaseId phase, Runnable callback) {
		int renderedItems = 0;
		...
		// We must clear the child state if we just entered the Render Phase, and there
		...
		/*
		 * Once rendering is done we need to make sure the child components are not
		 * still having client ids that use an index.
		 */
	...
	private static final class IndexedEvent extends FacesEvent {
		...
		public void processListener(FacesListener listener) {
			TabRepeat owner = (TabRepeat) this.getComponent();
	...
	public int encodeTabs(FacesContext faces, Runnable callback) throws IOException {
		if (!isRendered()) {
			return 0;
		}
		this.setDataModel(null);
		...
		return this.process(faces, PhaseId.RENDER_RESPONSE, callback);
	}
