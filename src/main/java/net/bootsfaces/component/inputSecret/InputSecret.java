...
public class InputSecret extends InputText {
	...
	/**
	 * By default, the value of the password field is never sent to the client. However, if you need to send the value to the client for some reason, you can set this flag to true. Please make sure that this is not a security hole. The password may be unreadable on the screen, but hackers can read it easily. <P>
	 * @return Returns the value of the attribute, or false, if it hasn't been set by the JSF file.
	 */
