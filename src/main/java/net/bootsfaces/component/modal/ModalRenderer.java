...
public class ModalRenderer extends CoreRenderer {
	...
	/**
	 * This methods generates the HTML code of the current b:modal. <code>encodeBegin</code> generates the start of
	 * the component. After the, the JSF framework calls <code>encodeChildren()</code> to generate the HTML code
	...
	private void initModal(ResponseWriter rw, String cId) throws IOException {
		...
		rw.write("$(function(){");
		rw.write("$('#CID').modal({ show: false });"
			.replace("CID", escapeClientId(cId)));
		rw.write("});");
